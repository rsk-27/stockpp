    ...
    function updateWishlistUI(wishlist) {
        ...
        wishlist.forEach(item => {
            ...
            row.innerHTML = `
                ...
                <td class="${item.change >= 0 ? 'profit' : 'loss'}">
                    ${formatPercentage(item.change)}
                </td>
                <td>
                    <button class="action-btn buy-btn" onclick="openTradeModal('${item.symbol}', 'buy', ${item.current_price})">
                        Buy
                    </button>
                    <button class="action-btn watch-btn" onclick="removeFromWishlist('${item.symbol}')">
                        Remove
                    </button>
                </td>
            `;
